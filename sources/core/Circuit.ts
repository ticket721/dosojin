...
export class Circuit {
    ...
    public async createGem<DosojinState = any>(
        initialGem: Gem = new Gem({}),
        initialArguments: DosojinState = null,
    ): Promise<Gem> {
        ...
        if (gem.operationStatus) {
            if (gem.operationStatus.layer < this.layers.length) {
                ...
                throw new CircuitError(
                    this.name,
                    `received Gem with invalid Operation Layer index ${gem.operationStatus.layer} (max ${this.layers
                        .length - 1})`,
                );
            }
        } else {
        ...
        if (gem.transferStatus) {
            ...
            if (
                ((gem.transferStatus.connector &&
                    gem.transferStatus.connector.status === TransferConnectorStatusNames.TransferComplete) ||
                    !gem.transferStatus.connector) &&
                gem.transferStatus.receptacle &&
                gem.transferStatus.receptacle.status === TransferReceptacleStatusNames.TransferComplete
            ) {
                gem.setRefreshTimer(null);

                const layer = gem.transferStatus.receptacle.layer;
                gem.setActionType('operation');
                await this.layers[layer].selectOperations(gem);

                if (gem.operationStatus.operation_list.length) {
                    return gem.setOperationStatus(OperationStatusNames.ReadyForOperation);
                } else {
                    await this.layers[layer].selectConnector(gem);
                    return gem.setConnectorStatus(TransferConnectorStatusNames.ReadyForTransfer);
                }
            }
            ...
            if (gem.transferStatus.receptacle) {
                if (gem.transferStatus.receptacle.layer < this.layers.length) {
                    ...
                    throw new CircuitError(
                        this.name,
                        `received Gem with invalid Receptacle Layer index ${
                            gem.transferStatus.receptacle.layer
            ...
            if (gem.transferStatus.connector) {
                if (gem.transferStatus.connector.layer < this.layers.length) {
                    ...
                    throw new CircuitError(
                        this.name,
                        `received Gem with invalid Connector Layer index ${
                            gem.transferStatus.connector.layer
                        } (max ${this.layers.length - 1})`,
                    );
                }
            }
