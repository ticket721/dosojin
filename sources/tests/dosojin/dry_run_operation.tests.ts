...
export function dry_run_operation_tests(): void {
    ...
    test('throw dosojin error when operation status is null', async () => {
        ...
        await expect(dosojin.dryRunOperation(gem)).rejects.toThrow();
        await expect(dosojin.dryRunOperation(gem)).rejects.toMatchObject({
            dosojin: dosojinName,
    ...
    test('dry run gem on operation when dosojin contains corresponding operation', async () => {
        const mockOperation1: SimpleOperationMock = new SimpleOperationMock('operation1', dosojin);
        ...
        await dosojin.dryRunOperation(gem);
