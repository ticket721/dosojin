...
export function get_connector_info_tests(): void {
    ...
    test('throw dosojin error when action type is invalid', async () => {
        ...
        await expect(dosojin.getConnectorInfo(gem)).rejects.toThrow();
    ...
    test('get connector info from connector when dosojin contains corresponding connector', async () => {
        when(mockGem.actionType).thenReturn('transfer');

        const expectedConnectorName: string = 'receptacle';
        ...
        await dosojin.getConnectorInfo(gem);

        verify(spiedConnector.getConnectorInfo(gem)).once();
    });
}
