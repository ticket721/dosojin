...
export function get_receptacle_info_tests(): void {
    ...
    test('throw dosojin error when action type is invalid', async () => {
        ...
        await expect(dosojin.getReceptacleInfo(gem)).rejects.toThrow();
    ...
    test('get receptacle info from receptacle when dosojin contains corresponding receptacle', async () => {
        when(mockGem.actionType).thenReturn('transfer');
        ...
        await dosojin.getReceptacleInfo(gem);

        verify(spiedReceptacle.getReceptacleInfo(gem)).once();
    });
}