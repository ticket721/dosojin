...
export function run_tests(): void {
    ...
    test('throw Error when the destination of payout is not a bank account', async () => {
        ...
        await expect(bankAccountPoConnector.run(gem)).rejects.toMatchObject(new Error(expectedErrorMessage));
    });
    ...
    test('Increase refreshTimer when payout is in transit', async () => {
        ...
        verify(
            mockGem.setState(deepEqual(dosojin), deepEqual({ refreshTimer: bankAccountPoConnector.refreshTimer })),
        ).once();
    });
