...
export function dry_run_tests(): void {
    ...
    test('throw Error when the payment method of paymentIntent is not a sepa debit', async () => {
        ...
        await expect(sepaDebitPiReceptacle.dryRun(gem)).rejects.toMatchObject(new Error(expectedErrorMessage));
    });
    ...
    test('Verify that receptacle status is set to transfer complete', async () => {
        ...
        verify(
            mockGem.addCost(
                ...
                deepEqual(
                    `|stripe| Checkout with sepa debit (${expectedBalanceTx.fee_details[0].type}): ${expectedBalanceTx.fee_details[0].description}`,
                ),
            ),
        ).once();
        ...
        verify(
            mockGem.addCost(
                ...
                deepEqual(
                    `|stripe| Checkout with sepa debit (${expectedBalanceTx.fee_details[1].type}): ${expectedBalanceTx.fee_details[1].description}`,
                ),
            ),
        ).once();
